...
logger = logging.get_logger("ned3pro_moveit.launch")
...
def generate_launch_description():
    ...
    move_group_node = Node(
        ...
        remappings=[
            ("/joint_states", "/quality_check/joint_states"), 
            ("/tf", "/quality_check/tf"),  
            ("/tf_static", "/quality_check/tf_static"),
        ],
    ...
    rviz_config = PathJoinSubstitution(
        [FindPackageShare("ned3pro_quality_check_manager"), "config", rviz_base]
    )
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
